...
public class TrashSpawner : MonoBehaviour
{
    ...
    void Update()
    {
        print(TrashinColumn.Count);
        t += Time.deltaTime;

        if (t % TrashManager.spawnRate == 0)
        {
            t = 0;
        ...
        {
            if (checkingSpriteBounds(i, player) && TrashinColumn[i].GetComponent<TrashPrefabScript>().move)
            {
            ...
            else if (checkingSpriteBounds(i, player) && !TrashinColumn[i].GetComponent<TrashPrefabScript>().move)
            {
            ...
            else if (TrashinColumn[i].GetComponent<TrashPrefabScript>().isItem && !TrashinColumn[i].GetComponent<TrashPrefabScript>().move) //destroy item if reach bottom
            {
                Destroy(TrashinColumn[i]);
                TrashinColumn.Remove(TrashinColumn[i]); 
            }
